	...
	Partial Public Class Form1
		...
		Private Sub schedulerControl1_AppointmentResizing(ByVal sender As Object, ByVal e As AppointmentResizeEventArgs) Handles schedulerControl1.AppointmentResized, schedulerControl1.AppointmentResizing
			If schedulerControl1.ActiveViewType = SchedulerViewType.Day Then
				...
				Dim shi As SchedulerHitInfo = schedulerControl1.ActiveView.ViewInfo.CalcHitInfo(mousePosition, True)
				Dim borderPos As Integer
				If e.ResizedSide = ResizedSide.AtStartTime Then
					borderPos = shi.ViewInfo.Bounds.Y
				Else
					borderPos = shi.ViewInfo.Bounds.Y + shi.ViewInfo.Bounds.Height
				End If
				If Math.Abs(mousePosition.Y - borderPos) > 1 Then
					Dim perc As Double = CDbl(mousePosition.Y - shi.ViewInfo.Bounds.Y) / CDbl(shi.ViewInfo.Bounds.Height)
